...
def group_images_by_satellite(image_paths):
    groups = {
        "Landsat_8_9": [],
        "Landsat_7_4_5_TM": [],
        "Landsat_1_5": []
    ...
    for path in image_paths:
        if re.search(r'LS8|LS9', path):
            groups["Landsat_8_9"].append(path)
        elif re.search(r'LS7|LT04|LT05', path):
            groups["Landsat_7_4_5_TM"].append(path)
        elif re.search(r'LM01|LM02|LM03|LM04|LM05', path):
            groups["Landsat_1_5"].append(path)

    return groups

def process_yearly_composites(input_dir, composite_dir, file_suffix, log_dir):
    """
    ...
    for year_folder in tqdm(year_folders, desc="Creating composites"):
        ...
        if image_files:
            grouped_images = group_images_by_satellite(image_files)
            for satellite_type, images in grouped_images.items():
                if images:
                    log_file = os.path.join(log_dir, f"{year}_{satellite_type}_composite.txt")
                    valid_images, log_entries = validate_geotiffs(images)
                    if log_entries:
                    ...
                    if valid_images:
                        output_file = os.path.join(composite_dir, f"{year}_{satellite_type}_median_composite.tif")
                        compute_median_composite(valid_images, output_file, log_file)
                        print(f"Created median composite for year {year} and satellite type {satellite_type}: {output_file}")
                    else:
                        print(f"No valid images found for year {year} and satellite type {satellite_type} after validation. See log file: {log_file}")
        else:
...
composite_directory = "landsat/Composites"
log_directory = "landsat/Log"
masked_tif = "_mclds_topshad_rad_srefdem_stdsref.tif"

if __name__ == "__main__":
    process_yearly_composites(output_directory, composite_directory, masked_tif, log_directory)

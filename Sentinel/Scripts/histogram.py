...
def generate_histograms(output_dir, surface_reflectance_end, radiance_end, raw_data_dir=None,
    ...
    if not os.path.exists(raw_histogram_output_dir):
        os.makedirs(raw_histogram_output_dir)
...
def generate_raw_histograms(raw_data_dir, raw_histogram_output_dir):
    date_pattern = re.compile(r"_\d{4}")
        ...
        for dir_name in dirs:
            if dir_name.endswith('.SAFE'):
                ...
                if raw_data:
                    ...
                    date_match = date_pattern.search(dir_name)
                    if date_match:
                        date_str = date_match.group()[1:]
                        date_folder = os.path.join(raw_histogram_output_dir, date_str)
                        if not os.path.exists(date_folder):
                            os.makedirs(date_folder)
                    else:
                        date_folder = raw_histogram_output_dir

                    title = f'Raw Sentinel-2 Histogram for {dir_name}'
                    output_path = os.path.join(date_folder, f"{dir_name}_raw_hist.png")
...
if __name__ == "__main__":
    ...
    raw_sentinel_dir = "sentinel/RAW"
    raw_histogram_output_dir = "sentinel/RAW/histograms"
